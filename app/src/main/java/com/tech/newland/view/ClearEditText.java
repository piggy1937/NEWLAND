...
public class ClearEditText extends AppCompatEditText {
    ...
    //设置右边图标的显示与隐藏
    public void setDefaultIconVisible(boolean visible) {
        //为空下面就没有意义，所以返回了
        if (mCompoundLeftDrawable == null)
            return;
        //不去给图标设置setBounds就可以设置图标的位置
        setCompoundDrawablesWithIntrinsicBounds( visible ? mCompoundLeftDrawable : null, mCompoundDrawables[1], mCompoundDrawables[2], mCompoundDrawables[3]);
    }
    /**
     * 开始清除按钮的显示动画
     */
    ...
    protected void onTextChanged(CharSequence text, int start, int lengthBefore, int lengthAfter) {
        ...
        if(text.length()>0) {
            ...
            //setDefaultIconVisible(!isVisible);
        }
    }
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
         if(event!=null&&keyCode== KeyEvent.KEYCODE_ENTER &&event.getAction() ==KeyEvent.ACTION_DOWN){
             mBus.onNext("1");
             return false;
         }
    ...
    public  ObservableSource<CharSequence> getObservable(){
        return (ObservableSource<CharSequence>) mBus.ofType(CharSequence.class);
    }
    public void  notify_input(){
        mBus.onNext("1");
    }
    public void  notify_empty(){
        mBus.onNext("0");
    }
}
